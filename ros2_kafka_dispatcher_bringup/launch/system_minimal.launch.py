...
def generate_launch_description():
    ...
    zenoh_sink_param_file = LaunchConfiguration("zenoh_sink_param_file")
    ...
    enable_zenoh_sink = LaunchConfiguration("enable_zenoh_sink")
    ...
    zenoh_sink_launch = PathJoinSubstitution(
        [
            FindPackageShare("ros2_kafka_dispatcher_bringup"),
            "launch",
            "zenoh_sink.launch.py",
        ]
    )
    ...
    return LaunchDescription(
        [
            ...
            DeclareLaunchArgument("enable_zenoh_sink", default_value="true"),
            ...
            IncludeLaunchDescription(
                PythonLaunchDescriptionSource(zenoh_sink_launch),
                condition=IfCondition(enable_zenoh_sink),
                launch_arguments={
                    "param_file": zenoh_sink_param_file,
                    "subscriptions_yaml": subscriptions_yaml,
                    "qos_depth": qos_depth,
                    "use_composition": "false",
                    "log_level": zenoh_sink_log_level,
                }.items(),
            ),
        ]
    )
