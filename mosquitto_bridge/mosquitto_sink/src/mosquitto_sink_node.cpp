...
// Attach the staged-latency envelope `_ts` to a JSON payload at publish time.
//   t0_ns  = message generated by dispatcher (from header.stamp, if present)
//   t1_ns  = message reaches the broker (this sink's publish wall-clock)
// Downstream components append t2_ns/t3_ns. Returns false if the payload is not
// parseable JSON (caller publishes the original payload unchanged).
bool apply_ts_envelope_to_json(std::string & json_payload, int64_t t1_ns)
{
  ...
  if (j.contains("header") && j["header"].is_object() &&
    j["header"].contains("stamp") && j["header"]["stamp"].is_object())
  {
    const auto & stamp = j["header"]["stamp"];
    if (stamp.contains("sec") && stamp.contains("nanosec")) {
      const int64_t sec = stamp["sec"].get<int64_t>();
      const int64_t nsec = stamp["nanosec"].get<int64_t>();
      if (sec != 0 || nsec != 0) {
        ts["t0_ns"] = sec * 1000000000LL + nsec;
      }
    }
  }
  ts["t1_ns"] = t1_ns;
    ...
    auto callback =
      [this, runtime_state](std::shared_ptr<rclcpp::SerializedMessage> msg) {
        ...
        if (runtime_state->payload_format == PayloadFormat::JSON) {
          ...
          // Stamp t1 (broker-ingress) into the `_ts` envelope just before publish.
          apply_ts_envelope_to_json(json_payload, static_cast<int64_t>(now_ns));
