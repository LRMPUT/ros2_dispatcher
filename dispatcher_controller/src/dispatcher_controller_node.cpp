...
: rclcpp::Node("dispatcher_controller", options)
{
  ...
  service_timeout_ = std::chrono::milliseconds(
    declare_parameter<int>("service_timeout_ms", static_cast<int>(service_timeout_.count())));
  ...
  dispatcher_controller::srv::GetStatus::Response::SharedPtr response)
{
  ...
  bool have_state = get_kafka_sink_state(state_id, state_error);
  response->kafka_sink_state = have_state ? (state_string(state_id)) : "unknown";
  response->streaming_active = have_state &&
  ...
  const std::vector<introspection_manager_msgs::msg::TopicInfo> & topics, std::string & error_out)
{
  ...
  uint8_t state{};
  if (!get_kafka_sink_state(state, error_out)) {
    return false;
  }
  ...
  if (state == lifecycle_msgs::msg::State::PRIMARY_STATE_ACTIVE) {
    ...
    if (!get_kafka_sink_state(state, error_out)) {
      error_out = "Failed to confirm kafka_sink state after deactivate: " + error_out;
      return false;
    }
  }
...
bool DispatcherControllerNode::get_kafka_sink_state(uint8_t & state_out, std::string & error_out)
{
  if (!get_state_client_->wait_for_service(service_timeout_)) {
    error_out = "get_state service not available";
    return false;
  }
  ...
  if (status != std::future_status::ready) {
    error_out = "Timeout waiting for kafka_sink state";
    return false;
  }
  state_out = future.get()->current_state.id;
  return true;
}
