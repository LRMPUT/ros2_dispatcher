...
// Validates `pkg/(msg|srv|action)/Type` strings before any type-support library
// is dlopen()'d. The ROS type name on an incoming Zenoh sample is attacker-
// controllable (anyone on the Zenoh network can publish), so this is a security
// gate, not just a sanity check. Kept as a local copy (mirrors
// kafka_client/ros_type_validation.hpp) to avoid coupling zenoh_source to the
// Kafka stack.

#ifndef ZENOH_SOURCE__ROS_TYPE_VALIDATION_HPP_
#define ZENOH_SOURCE__ROS_TYPE_VALIDATION_HPP_
...
#endif  // ZENOH_SOURCE__ROS_TYPE_VALIDATION_HPP_
